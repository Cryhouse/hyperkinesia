...
# Get performance of binary predictor, resample test data

def main():
    labels_all = parsing.parse_all_labels()
    filename_df = pipeline.get_filename_df(labels_all, RAW_DATA_DIR, t="bc", force=True)
    
    Y_full_class, Y_full_reg, Y_full_per, Y_full_jon = pipeline.get_Ys(labels_all)
    
    X_full = pipeline.get_X_full(filename_df, labels_all, Y_full_reg, force=True, t="bc")
